...
- **Asynchroniczna komunikacja** – Domain events na Kafka + schematy Avro, Outbox Pattern, Event Sourcing dla krytycznych zdarzeń (rezerwacje, płatności). Dla synchronicznych wywołań gRPC + Circuit Breaker (Envoy + Tenacity w klientach FastAPI).
